class PriorityQueue {
  ...
  compare(taskA, taskB) {
    ...
    if (priorityOrder[taskA.priority] !== priorityOrder[taskB.priority]) {
      return priorityOrder[taskA.priority] < priorityOrder[taskB.priority]; // High priority comes first
    }
    return new Date(taskA.createdAt) < new Date(taskB.createdAt); // Older tasks come first
  }
